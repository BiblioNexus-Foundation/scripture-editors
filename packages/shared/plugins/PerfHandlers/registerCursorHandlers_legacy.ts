...
export function registerCursorHandlers(editor: LexicalEditor) {
  function $getSelectionData(selection: BaseSelection) {
    ...
    if (endNode) {
      ...
      isZeroWidthSpace =
        $isTextNode(endNode) && endNode.getTextContent() === CURSOR_POSITION_HELPER_CHAR;
    }
        ...
        if (currSelectionData.isZeroWidthSpace) {
          ...
          editor.update(() => {
            (currSelectionData.endNode as TextNode).setTextContent(
              currSelectionData.endNode.getTextContent().replace(CURSOR_POSITION_HELPER_CHAR, ""),
            );
          });
        }
      });
      ...
      if (prevSelectionData.isZeroWidthSpace) {
        ...
        editor.update(() => {
          (prevSelectionData.endNode as TextNode).setTextContent(
            prevSelectionData.endNode.getTextContent().replace(CURSOR_POSITION_HELPER_CHAR, ""),
          );
        });
      }
    });
  ...
  const removeZeroWidthSpace = (textNode: TextNode) => {
    ...
    if (textContent === CURSOR_POSITION_HELPER_CHAR) {
      ...
      editor.update(() => {
        textNode.setTextContent(textContent.replace(CURSOR_POSITION_HELPER_CHAR, ""));
      });
    }
  };
  ...
  const insertTextNode = (
    ...
    text = CURSOR_POSITION_HELPER_CHAR,
  ) => {
    editor.update(
      () => {
