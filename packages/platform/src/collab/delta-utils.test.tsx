  ...
  describe("Retain Operations", () => {
    ...
    it("should retain embedded chapter with attributes", async () => {
      ...
      editor.getEditorState().read(() => {
        ...
        if (!$isSomeChapterNode(ch2)) throw new Error("c2 is not SomeChapterNode");
    ...
    it("should transform text to CharNode and apply top-level formats to inner text", async () => {
      ...
      const cid = "char-id-1";
      const ops: Op[] = [
        { retain: 1 + prefix.length }, // Retain 1 for the ParaNode.
        {
    ...
    it("should fallback to standard attribute application if char.style is missing", async () => {
      ...
      const ops: Op[] = [
        { retain: 1 }, // Retain 1 for the ParaNode.
        {
          retain: text.length,
    ...
    it("should fallback to standard attribute application when retain targets an existing embed for char transformation", async () => {
      ...
      const ops: Op[] = [
        { retain: 1 + textBefore.length }, // Retain 1 for the ParaNode.
        {
  ...
  describe("Insert Operations", () => {
    ...
    it("should insert an embed when the targetIndex is out of bounds", async () => {
      ...
      editor.getEditorState().read(() => {
        ...
        const v10Node = paraNode.getLastChild();
        if (!$isSomeVerseNode(v10Node)) throw new Error("Last child of para is not SomeVerseNode");
    ...
    it("should insert an embed between two existing embed nodes", async () => {
      ...
      const ops: Op[] = [{ retain: 1 + 1 }, { insert: v2Embed }];
    ...
    it("should insert milestone embeds in empty para", async () => {
      ...
      const ops: Op[] = [{ retain: 1 }, { insert: msStartEmbed }];
    ...
    it("should sequentially insert multiple items into an empty document", async () => {
      ...
      editor.getEditorState().read(() => {
        const root = $getRoot();
        expect(root.getChildrenSize()).toBe(2); // Chapter1, existing implied-para
        ...
        expect(impliedParaNode.getChildrenSize()).toBe(3);
        ...
        const v2Node = impliedParaNode.getChildAtIndex(2);
        if (!$isSomeVerseNode(v2Node))
          throw new Error("Child of impliedParaNode is not SomeVerseNode");
    ...
    it("should sequentially insert multiple embeds into an 'empty' document", async () => {
      const { editor } = await testEnvironment();
      const ch1Embed = { chapter: { number: "1", style: "c" } };
      const paraQ1Embed = { para: { style: "q1" } };
      const ch2Embed = { chapter: { number: "2", style: "c" } };
      const paraQ2Embed = { para: { style: "q2" } };
      const v1Embed = { verse: { number: "1", style: "v" } };
      ...
      editor.getEditorState().read(() => {
        const root = $getRoot();
        expect(root.getChildrenSize()).toBe(5); // Chapter1, ParaQ1, Chapter2, ParaQ2, existing empty implied-para

        const chapter1Node = root.getChildAtIndex(0);
        if (!$isSomeChapterNode(chapter1Node)) throw new Error("Child 0 is not SomeChapterNode");
        expect(chapter1Node.getNumber()).toBe("1");

        const paraQ1Node = root.getChildAtIndex(1);
        if (!$isParaNode(paraQ1Node)) throw new Error("Child 1 is not a ParaNode");
        expect(paraQ1Node.getMarker()).toBe("q1");
        expect(paraQ1Node.getChildrenSize()).toBe(0); // ParaQ1 should be empty

        const chapter2Node = root.getChildAtIndex(2);
        if (!$isSomeChapterNode(chapter2Node)) throw new Error("Child 2 is not SomeChapterNode");
        expect(chapter2Node.getNumber()).toBe("2");

        const paraQ2Node = root.getChildAtIndex(3);
        if (!$isParaNode(paraQ2Node)) throw new Error("Child 3 is not a ParaNode");
        expect(paraQ2Node.getMarker()).toBe("q2");
        expect(paraQ2Node.getChildrenSize()).toBe(3);

        const v1Node = paraQ2Node.getChildAtIndex(0);
        if (!$isSomeVerseNode(v1Node)) throw new Error("Child of ParaQ2 is not SomeVerseNode");
        expect(v1Node.getNumber()).toBe("1");

        const v1TextNode = paraQ2Node.getChildAtIndex(1);
        if (!$isTextNode(v1TextNode)) throw new Error("Child of ParaQ2 is not a TextNode");
        expect(v1TextNode.getTextContent()).toBe("v1 text");

        const milestoneNode = paraQ2Node.getChildAtIndex(2);
        if (!$isMilestoneNode(milestoneNode))
          throw new Error("Child of ParaQ2 is not a MilestoneNode");
        expect(milestoneNode.getMarker()).toBe("ts-s");
        expect(milestoneNode.getSid()).toBe("TS1");

        const impliedParaNode = root.getChildAtIndex(4);
        if (!$isImpliedParaNode(impliedParaNode))
          throw new Error("Child 4 is not a ImpliedParaNode");
        expect(impliedParaNode.getChildrenSize()).toBe(0);
      });
    });
  });
});
